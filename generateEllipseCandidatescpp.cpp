						...
						double prec)
{
	...
	if (reg_size == NULL)
		error("region_grow: invalid point2ier 'reg_size'.");
	if (reg_angle == NULL)
		error("region_grow: invalid point2ier 'reg_angle'.");
...
{
	image_double image;
	ntuple_list out = new_ntuple_list(8); // x1,y1,x2,y2,dx,dy,distance,polarity
	double *return_value;
	...
	ofstream outfile;
	outfile.open(filename + "arcpoints.csv");
	outfile << "arc,x,y" << endl;

	ofstream regionout;
	regionout.open(filename + "regionout.txt");
	...
	infoout.open(filename + "info.txt");
	infoout << xsize << " " << ysize;
	/* Number of Tests - NT
  ...
  */
	...
	for (; list_p_temp != NULL; list_p_temp = list_p_temp->next)
         ...
         */
			...
			{
				outfile << seed_cnt << "," << reg[i].x << "," << reg[i].y << endl;
			}

			// cout << "regsize = " << reg_size << endl;
			...
			if (ls_count == 1) //清空内容
			{
				file1.open(filename + "coor.txt", ios::out | ios::trunc);
				file1.close();
				file2.open(filename + "reg.txt", ios::out | ios::trunc);
				file2.close();
			}

			file1.open(filename + "coor.txt", ios::app);
			file1 << main_rect.x1 << '\t' << main_rect.y1 << '\t' << main_rect.x2 << '\t' << main_rect.y2 << '\t' << (main_rect.theta) << endl;
			file1.close();

			if (ls_count == 1) //保持第1根线段的区域
			{
				file2.open(filename + "reg.txt", ios::app);
				for (i = 0; i < reg_size; i++)
			...
			if (region != NULL)
			{
				...
				for (i = 0; i < reg_size; i++)
				{
					...
					if (1)
					{
						regionout << reg[i].x + reg[i].y * region->xsize << " " << reg[i].x << " " << reg[i].y << " " << ls_count << endl;
					}
				}
			}
		}
...
int fitEllipse(point2d *dataxy, int datanum, double *ellipara)
{
	...
	// eig(S,C) eig(inv(S)*C)
	MatrixXd C(6, 6);
	C.topRightCorner(3, 3).setZero();
	C.bottomLeftCorner(3, 6).setZero();
	C.topLeftCorner(3, 3) << 0, 0, 2,
		0, -1, 0,
		2, 0, 0;
	...
	GeneralizedEigenSolver<MatrixXd> S_e;
	S_e.compute(S_m, C);
	info = S_e.info();
	complex<double> a[6];
	for (int i = 0; i < 6; i++)
	{
		a[i] = (S_e.alphas().data())[i];
		alphar[i] = a[i].real();
		alphai[i] = a[i].imag();
		beta[i] = S_e.betas().data()[i];
	}
	for (int i = 0; i < 36; i++)
	{
		vr[i] = (S_e.eigenvectors().data())[i].real();
		// cout << a[i].real() << endl;
	}
	// info = LAPACKE_dggev(LAPACK_ROW_MAJOR,'N','V',6,S,6,C,6,alphar,alphai,beta,vl,6,vr,6);
	//注意S为对称矩阵，故转置后等于本身，变成列优先，S可以不变
	// dgeev_(&JOBVL,&JOBVR,&fitN,S,&fitN,C,&fitN,alphar,alphai,beta,vl,&fitN,vr,&fitN,fitWork,&workLen,&info);
...
void generateEllipseCandidatesCpp(Mat src, const int edge_process_select, const int specified_polarity, image_double_s &candiDouble)
{
	// ofstream drcfile,inputfile;
	// drcfile.open(filename+"drc.txt");
	// inputfile.open(filename+"input.txt");

	// for out

	int imgx = src.rows;
	int imgy = src.cols;
	uchar *inputimg = new uchar[imgy * imgx];
	if (src.isContinuous())
		inputimg = src.data;
	...
	for (int c = 0; c < imgx; c++)
	{
		for (int r = 0; r < imgy; r++)
		{
			...
			//    inputfile << (int)inputimg[r+c*imgy]<< " ";
		}
		// inputfile << endl;
	}
	...
	// out: x1,y1,x2,y2,dx,dy,distance,polarity

	groupLSs(out/*input*/, n/*output*/, reg/*output*/, reg_x/*output*/, reg_y/*output*/, &groups/*output*/);  //分组
	free(reg);									   //释放内存
	...
	infoout << " " << n << " " << (int)groups.size() << " ";
	// for (int i = 0; i < groups.size(); i++)
	// {
	// 	for (int j = 0; j < groups[i].size(); j++)
	// 	{
	// 		cout << "*group data " << *(groups[i].data()+j) << ",";
	// 	}
	// 	cout << endl;
	// }
	...
	if (pairGroupList != NULL)
	{
		...
		infoout << pairGroupList->length << " " << candidates_num;
		infoout.close();
		ofstream candioutfile;
		candioutfile.open(filename + "candidates.txt");
		// candioutfile << "x,y,a,b,phi" << endl;
		for (int i = 0; i < candidates_num; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				candioutfile << *(candidates + i * 5 + j) << ",";
			}
			candioutfile << *(candidates + i * 5 + 4) << endl;
		}
		candioutfile.close();
	}
	else
	{
		cerr << "pairGroupList NULL" << endl;
	}
	candiDouble.data = candidates;
	candiDouble.xsize = 5;
	candiDouble.ysize = candidates_num;
	return;
...
int semimain(int argc, char **argv)
{
	...
	// cout << parser.get<String>("@input")+".jpg" << endl;
	filename = parser.get<String>("@input");
	cout << filename << endl;
	Mat src = imread(samples::findFile(parser.get<String>("@input") + ".jpg"), IMREAD_COLOR);
	Mat gray;
	cvtColor(src,gray,COLOR_BGR2GRAY);
	image_double_s candidates;
	// Mat dst(3,3,CV_8UC1);
	// int data[9] = {10,20,30,40,50,60,70,80,90};
	// uchar * udata = new uchar[9];
	// // dst.data =

	// // cout << "STYPE:" << src.type() << endl;
	// // Mat dst( 125, 125, CV_64FC4, Scalar(0,0,255));
	// int i, j;
	// for ( i=0; i<9 ; i++)
	// {
	//     // cout << "element " << i << " = " << (int) *(dst.data+i) << endl;
	//     *(udata + i) = uchar(*(data+i));
	//     cout << "udata " << i << " = " << (int) *(udata+i) << endl;
	// }

	// dst.data = udata;
	// cout << "DTYPE:" << dst.type() << endl;
	...
	// cout << "samples::findFile(parser.get<String>(input))" << samples::findFile(parser.get<String>("@input")) << endl;
	// cout << "parser.get<String>( @input ) =" << parser.get<String>("@input") << endl;
	generateEllipseCandidatesCpp(gray, 0, 0, candidates);
	// 设置窗口
	// Mat img = Mat::zeros(Size(800, 600), CV_8UC3);
	
	// src.setTo(255);              // 设置屏幕为白色
	// int thickness = 1;
	// int lineType = 8;
	double angle = 30;  //椭圆旋转角度
	for (int i = 0; i < candidates.ysize; i++)
    {
        ellipse(src, Point(*(candidates.data + i * candidates.xsize + 1), *(candidates.data + i * candidates.xsize)), Size(*(candidates.data + i * candidates.xsize + 3), *(candidates.data + i * candidates.xsize + 2)), -*(candidates.data + i * candidates.xsize + 4) * 180/M_PI, 0, 360, Scalar(0, 255, 0),2);
    }
	// ellipse(src, Point(100, 100), Size(90, 60), angle, 0, 360, Scalar(255, 255, 0));
	imshow("画板", src);
	int key;
	while ((key = waitKey()) != 'q')
	{
		if (key == 's')
		{
			imwrite(filename+"result.jpg",src);
		}
